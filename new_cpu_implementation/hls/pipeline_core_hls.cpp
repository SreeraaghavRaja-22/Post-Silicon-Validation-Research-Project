...
void clock_cycle(){
    ...
    if(EX_MEM.valid){
        if(EX_MEM.opcode == 0x03){ // Load Word
            // Indexing requires explicit declaring the int type
            MEM_WB.outVal = DataMemory[(int)EX_MEM.outVal]; // load from data memory for LW
    ...
    // Change the operand types so that they match the types for val1/val2/outVal
    if(ID_EX.valid){
        // Forwarding MUXes for Data Forwarding
        ap_int<32> operand1 = ID_EX.val1; // Stale value from RegFile
        ...
        // Operand 2
        ap_int<32> operand2 = ID_EX.val2; // Stale value from RegFile
        ...
        switch((int)ID_EX.opcode){
                ...
                switch((int)ID_EX.funct3){
                    ...
                    case 0x5: // SRL/SRLI (funct7=0x00) or SRA/SRAI (funct7=0x20)
                        if(ID_EX.funct7 == 0x20)
                            aluResult = (int32_t)operand1 >> ((int)aluOperand2 & 0x1F);                    // arithmetic shift
                        else
                            aluResult = (int32_t)((uint32_t)(int32_t)operand1 >> ((int)aluOperand2 & 0x1F)); // logical shift
                        break;
                    case 0x6: // OR / ORI
                        aluResult = operand1 | aluOperand2;
                        break;
            ...
            case 0x63: { // Branch -- resolved here in EX
                ...
                switch((int)ID_EX.funct3){
                    ...
                    // helps to accurately compare different integer values based on magnitude
                    case 0x6: takeBranch = ((uint32_t)(int32_t)operand1 <  (uint32_t)(int32_t)operand2); break; // BLTU
                    case 0x7: takeBranch = ((uint32_t)(int32_t)operand1 >= (uint32_t)(int32_t)operand2); break; // BGEU
                    default: break;
                }
                if(takeBranch){
                    // these internal values need to be signed because jumps can be forward or backward
                    PC = (uint32_t)((int32_t)ID_EX.pc + (int32_t)ID_EX.imm);   // redirect fetch to branch target
                    IF_ID.reset();     // squash the wrong-path instruction already fetched
                }
                break;
            }
            ...
            case 0x67: // JALR -- unconditional jump to (rs1+imm) with LSB cleared, link register = pc+4
                aluResult = (int32_t)ID_EX.pc + 4;
                PC = ((uint32_t)((int32_t)operand1 + (int32_t)ID_EX.imm)) & ~1u; // needs the ~1u to make sure bit 0 = 0 so that all the results are even
                IF_ID.reset();         // squash the wrong-path instruction already fetched
                break;
    ...
    if(!pipeline_stall){
        if(IF_ID.valid){
            ...
            // ID_EX = PipelineLatch();
            ID_EX.reset(); // same thing but is better
        }
    }
    else{
        // Insert a Bubble if the pipeline is stalled
        ID_EX.reset(); // NOP = Bubble
    }

    // 5. IF Stage: fetch next instruction from InstructionMemory using current PC value and increment PC
    if(!pipeline_stall){
        IF_ID.instr = InstructionMemory[(uint32_t)PC>>2]; // get subsequent instruction
        IF_ID.pc = PC;
        IF_ID.valid = true;
        PC = (uint32_t)PC + 4; // increment PC by 32 bits
    }
}
